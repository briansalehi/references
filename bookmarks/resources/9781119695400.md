...
> |---|---|
> |Input Iterator|Read-only access `operator*` `operator->`, forward only `operator++`, assignable `operator=`, copyable and comparable for equality `operator==` `operator!=`|
> |Output Iterator|Read-only access `operator*` (no `operator->` access), forward only `operator++`, assignable `operator=`, copyable, no comparability supported|
> |Forward Iterator|Same as Input Iterator but with default constructor: read-only access `operator*` `operator->`, forward only `operator++`, assignable `operator=`, copyable and comparable for equality `operator==` `operator!=`|
> |Bidirectional Iterator|Same as Forward Iterator plus `operator--`: read-only access `operator*` `operator->`, bidirectional `operator++` `operator--`, assignable `operator=`, copyable and comparable for equality `operator==` `operator!=`|
> |Random Access Iterator|Same as Bidirectional Iterator plus comparability and index operator: read-only access `operator*` `operator->`, bidirectional `operator++` `operator--`, assignable `operator=` `operator+=` `operator-=`, arithmetic `operator+` `operator-`, copyable and comparable `operator==` `operator!=` `operator>` `operator<` `operator<=` `operator>=`, element-wise access `operator[]`|
> |Contiguous Iterator|Random access capability and logically adjacent elements of the container must be physically adjacent in memory|
