...
Now, let’s say that we want to block certain IP addresses from reaching the Secure Shell port of this machine.
...
Another really cool thing to note is how we’ve mixed IPv4 (ip) rules with IPv6 (ip6) rules in the same configuration file.
Also, unless we specify otherwise, all the rules that we create will apply to both IPv4 and IPv6.
That’s the beauty of using an inet-type table.

Let’s say that you’ve observed an attack in progress, and you need to stop it quickly without bringing down the system.
With an nft command, you can create a custom rule on the fly that will block the attack.

you could just do everything directly from the Bash shell, prefacing every action you want to perform with nft, followed by the nft subcommands.
