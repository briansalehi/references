...
**Note:** first parent (a.k.a the starting process) should wait for its child, or the second parent in this hierarchy.  
**Note:** first child (a.k.a. the second parent) should create a new session to escape controlling terminal and create a pid file in `/var/run/` containing the process ID of its child (a.k.a. daemon).  
**Note:** second child (a.k.a. daemon) should set `umask(2)` and change its directory `chdir(2)` to the root directory.  
...
**Note:** their systemd(1) service file has `Type` of *simple* and `Restart` of *on-failure*.
