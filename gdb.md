...
GDB tip: To debug with GDB, compile with -g. This generates DWARF information, which is used by the debugger to understand what the program is doing. It doesn’t affect generated code at all, just adds debug information to the binary. Use -O to compile with optimizations, but this can adversely affect the debug experience i.e. -O and -g are orthogonal and independent. To get a reasonable level of performance and a good debug experience, use -Og. And to get an even better debug experience, use -g3 rather than just -g. You can use the 'readelf' utility to look at the debug info (see example below)

Learn more on my free gdbWatchPoint resource portal: https://hubs.ly/Q01k9ZZ_0

##### https://www.linkedin.com/posts/gregthelaw_gdb-debugging-cplusplus-activity-6965701577579888640-rLGq/
###### <Card>

👉 Tip #2 
TUI (Text User Interface) mode allows you to see the context of what you’re debugging! Start with gdb -tui, or type `layout src` to switch to the TUI mode. Or Ctrl–x-a.
(gdb) layout src
...
👉 Tip #8 : Hardware watchpoints tracks any changes to a variable. Actually, changes to an expression. e.g. watch 𝚙->𝚊 will stop when 𝚙 changes and/or when 𝚊 changes (unless p->a evaluates to the same thing) 
...
👉 Tip #9: Location watchpoints: tracks a particular location in memory rather than the value of an expression. Useful to find memory corruption errors e.g. 𝚠𝚊𝚝𝚌𝚑 𝚏𝚘𝚘->𝚊 won’t stop if 𝚏𝚘𝚘 changes, only when the underlying memory does, even if, say, 𝚏𝚘𝚘 is local and goes out of scope.
...
👉 Tip #10: Location watchpoints and $1 style convenience variables work nicely together:
...
👉 Tip #11: Software watchpoints: If GDB cannot use hardware support for watchpoints (e.g. on x86 a big or misaligned structure) it will fall back to software watchpoints. Beware though: these are very slow as GDB single-steps repeatedly to see when the data changes eg: watch $eax will use software watchpoints.
...
👉 Tip #12: A breakpoint makes a program stop if a certain point in the program is reached. For each breakpoint, add conditions to control whether the program stops. Set breakpoints with the break command. A watchpoint is a special breakpoint that stops your program when the value of an expression changes. 
...
👉 Tip #13: Breakpoints: If the program has multiple functions with same name but different argument types, setting a breakpoint on any one of them can be done as 𝚋𝚛𝚎𝚊𝚔 𝚏𝚞𝚗𝚌_𝚗𝚊𝚖𝚎(𝚝𝚢𝚙𝚎)
...
👉 Tip #14: GDB provides the ability to use regex to specify breakpoints using 𝚛𝚋𝚛𝚎𝚊𝚔
...
👉 Tip #15: Conditional breakpoints - condition x controls when breakpoint x (already set) should be triggered e.g. 𝚌𝚘𝚗𝚍𝚒𝚝𝚒𝚘𝚗 𝟹 𝚕==𝟺𝟷
...
👉 Tip #17: You can extract a slice of an array in GDB e.g. 
...
👉 Tip #18: When you set a watchpoint (e.g. `watch foo`), GDB will try to use hardware watchpoints: nice and fast. But if you watch something bigger than the hardware supports, you get software watchpoints: these are horribly slow. You have to pay close attention to spot the difference.
...
👉 Tip #19: Usually a watchpoint watches an expression. Use -location (or -l) to watch certain bytes in memory. 
...
👉 Tip #20: Watching the expression will stop only when the result of the entire expression changes. The example here shows GDB doesn’t stop when line 14 executes because although 𝚒 has changed, 𝚖𝚢_𝚊𝚛𝚛𝚊𝚢[𝚒] still evaluates to 42. It does stop after line 15 executes though. [see image below]
...
👉 Tip #21: Use 𝚌𝚘𝚖𝚖𝚊𝚗𝚍𝚜 to give a list of commands to execute whenever a breakpoint is hit. See example in screenshot below.

##### https://www.linkedin.com/posts/gregthelaw_debugging-gdb-cplusplus-activity-6983751540926783488-kBMK/
###### <Card>

👉 Tip #22: Use the 𝚌𝚘𝚖𝚖𝚊𝚗𝚍𝚜 to set a new breakpoint (something Visual Studio calls 'dependent breakpoints') 

##### https://www.linkedin.com/posts/gregthelaw_debugging-gdb-cplusplus-activity-6984166245377921024-5mUo/
###### <Card>

👉 Tip #23: Use the magic silent command in your list of commands to prevent output appearing from the evaluation of your breakpoint commands.
...
👉 Tip #24: If you can’t use artificial arrays (e.g. complex data structure), you can update convenience variables inside a print expression and then hit enter multiple times to iterate through. (Note 𝚒 and $𝚒 are different things here).
...
👉 Tip #25: The forward-search command finds source matching a regex and puts the line number in the convenience variable $_ which you can then use to set a breakpoint.

##### https://www.linkedin.com/posts/gregthelaw_debugging-gdb-cplusplus-activity-6989246448009629696-NYet/
###### <Card>

👉 Tip #26: Turn on pretty print to make data structures much easier to read when inspecting them while debugging
...
👉 Tip #27: Printing can be customised in GDB to change the number of elements shown in strings/arrays, show particular slices of elements, and even change the output format.
...
💡 GDB tip
👉 Tip #28: Using "disassemble /s " allows you to view the assembly of a specific function interspersed with the original code of the function in C/C++, allowing you to find instructions more easily.
...
👉 Tip #29: You can use 𝚝𝚞𝚒 𝚎𝚗𝚊𝚋𝚕𝚎 to view the source code of your program alongside the GDB terminal. You can also use 𝚕𝚊𝚢𝚘𝚞𝚝 𝚜𝚙𝚕𝚒𝚝 to enable a view of the assembly and 𝚝𝚞𝚒 𝚛𝚎𝚐 𝚐𝚎𝚗𝚎𝚛𝚊𝚕 to view the registers in this GUI as well.
...
👉 Tip #30: When debugging a multithreaded program, you can apply the same commands to all threads simultaneously with “thread apply all ”, for example:
...
👉 Tip #31: For bugs that scarcely appear, you can use GDB commands to re-run the program until it fails [see screenshot below]
...
💡 GDB tip of the day
👉 Tip #32: Using the default TUI can come with some caveats, like your arrow keys being mapped to the C source file. It can be customised even more with some extra commands, such as 𝚏𝚘𝚌𝚞𝚜 which will change the active window (for example 𝚌𝚖𝚍 to focus on the GDB terminal).
...
👉 Tip #33: Using the default TUI can come with some caveats, like your arrow keys being mapped to the C source file. You can customise the size of a window using 𝚠𝚒𝚗𝚑𝚎𝚒𝚐𝚑𝚝 + to increase it by lines, or - to reduce the size. Use these extra customisation features to optimise your debugging workflow.
...
💡 My GDB tip of the day
Tip #34: If you have a program that takes a particularly long time to run, setting a checkpoint when you’re closer to the error can save lots of debugging time if you’re unsure where you should be looking. Restarting from the checkpoint will put GDB back to exactly that state of execution, instead of having to rerun the program from the beginning.
...
💡 My GDB tip of the day
Tip #35: As well as inspecting the values of variables with print, you can evaluate expressions while debugging to see how it behaves with different inputs at runtime e.g. “print foo(arg1, arg2)”.
...
💡 My GDB tip of the day
Tip #36: You can compile and immediately execute code on demand in the context of the current inferior using 𝚌𝚘𝚖𝚙𝚒𝚕𝚎 𝚌𝚘𝚍𝚎 𝚙𝚛𝚒𝚗𝚝𝚏 (“𝚑𝚎𝚕𝚕𝚘 𝚠𝚘𝚛𝚕𝚍\𝚗”); or 𝚌𝚘𝚖𝚙𝚒𝚕𝚎 𝚏𝚒𝚕𝚎 . Once it's finished executing the compiled code is removed and new variables are deleted.
...
💡 My GDB tip of the day
Tip #37: You can force a function to return prematurely with the return command, or return something in particular with 𝚛𝚎𝚝𝚞𝚛𝚗 . It discards the selected stack frame and any inside it, and the debugger stays in a state as if the function has just returned that expression.
...
💡 GDB Tip #38: Just found out a nice way of debugging multithreaded applications; after 𝚜𝚎𝚝 𝚜𝚌𝚑𝚎𝚍𝚞𝚕𝚎𝚛-𝚕𝚘𝚌𝚔𝚒𝚗𝚐 𝚘𝚗 you can make threads all progress one step at a time with 𝚝𝚑𝚛𝚎𝚊𝚍 𝚊𝚙𝚙𝚕𝚢 𝚊𝚕𝚕 𝚜𝚝𝚎𝚙, instead of one thread progressing more than another when you do run 𝚜𝚝𝚎𝚙 in one inferior.
...
💡 GDB Tip #39: You can inspect the state of a program after it’s crashed using core dumps. 𝚐𝚍𝚋 will open the program in GDB and let you view the stack frame and the state of variables and registers at the point the program crashed.
...
💡 GDB Tip #40: If you want to debug a program running on a smaller device with limited space, or are working with a real-time system where the development happens on a different machine, you can use the gdbserver program.
...
💡 GDB Tip #42: You can run and debug Python files! Using 𝚐𝚍𝚋 -𝚊𝚛𝚐𝚜 𝚙𝚢𝚝𝚑𝚘𝚗 𝚖𝚊𝚒𝚗.𝚙𝚢 you can run the Python script and inspect the backtrace when it crashes. Particularly helpful for Python scripts that segfault.
...
💡 GDB Tip #43: Did you know you can inspect variables in different output formats, for example binary or hexadecimal, using 𝚙𝚛𝚒𝚗𝚝 / ?
...
💡 GDB Tip #44: Skip through loops using the 𝚞𝚗𝚝𝚒𝚕 command! Unlike next at the end of a loop, which loops back to the start, 𝚞𝚗𝚝𝚒𝚕 will continue the program execution until the loop exits.
...
💡 GDB Tip #45: Alongside existing tools facilitating the practice, GDB even has its own reverse-debugging feature built in! Use record at the start of execution, and you can use reverse-step or reverse-continue to go back to specific points while debugging. It’s limited and slow, but it can still be useful.
...
💡 GDB Tip #46: Adding to my previous tip on GDB's reverse-debugging feature (ref. https://hubs.ly/Q01FzGyV0): To save some time with writing commands you can use 𝚜𝚎𝚝 𝚎𝚡𝚎𝚌-𝚍𝚒𝚛𝚎𝚌𝚝𝚒𝚘𝚗 [𝚏𝚘𝚛𝚠𝚊𝚛𝚍 | 𝚛𝚎𝚟𝚎𝚛𝚜𝚎] and the usual 𝚌𝚘𝚗𝚝𝚒𝚗𝚞𝚎/𝚜𝚝𝚎𝚙 commands will go in that direction instead! (see screenshot below)
...
💡 GDB Tip #47: You can get information about the memory mapping of the current process using 𝚒𝚗𝚏𝚘 𝚙𝚛𝚘𝚌 𝚖𝚊𝚙𝚙𝚒𝚗𝚐𝚜 - helpful to understand how the memory is allocated and managed.
...
💡 GDB Tip #48: Set catchpoints using 𝚌𝚊𝚝𝚌𝚑 to catch program events, like specific exceptions, for when you know an error will happen but you don’t know where!
...
💡 GDB Tip #49: To help automate your debugging you can write multiple commands into a command file, and run it with "source ". Any commands that usually require confirmation will proceed automatically. 

##### https://www.linkedin.com/posts/gregthelaw_debugging-gdb-cplusplus-activity-7045092405229842433-usIY/
###### <Card>

💡 GDB Tip #50: If you want to save the output of GDB to a file while debugging as well as printing to console, you can use 𝚜𝚎𝚝 𝚕𝚘𝚐𝚐𝚒𝚗𝚐 𝚎𝚗𝚊𝚋𝚕𝚎𝚍 𝚘𝚗 to store the output persistently!
...
💡 GDB Tip #51: You can pipe the output from commands directly to external shell commands from within GDB! This means you can use grep, wc, awk on the output of a specific GDB output. For example: 𝚙𝚒𝚙𝚎 𝚙 $𝟷 | 𝚠𝚌 -𝚕 (or 𝚛𝚞𝚗 | 𝚌𝚘𝚠𝚜𝚊𝚢)
...
💡 GDB Tip #52: Most commands in GDB have an abbreviated form, for example 𝚗 for 𝚗𝚎𝚡𝚝, 𝚋 for 𝚋𝚛𝚎𝚊𝚔, or 𝚜𝚒 for 𝚜𝚝𝚎𝚙𝚒. These can be used in place of typing out the full command. There are even abbreviations for some commands and their arguments, like 𝚝𝚊𝚊 for 𝚝𝚑𝚛𝚎𝚊𝚍 𝚊𝚙𝚙𝚕𝚢 𝚊𝚕𝚕.
...
💡 GDB Tip #53: You can set multiple breakpoints on functions using regular expressions with the 𝚛𝚋𝚛𝚎𝚊𝚔 command. This can help if you’ve got multiple functions to break on with a consistent naming scheme, for example.
...
💡 GDB Tip #54: As well as running python inside GDB, you can run GDB commands from python! With the 𝚐𝚍𝚋 module you can use python to automate your debugging with more customisation than command files.
...
💡 GDB Tip #55: Another python integration tip: you can use python to write pretty-printers, extending the functionality of or replacing the in-built GDB pretty printer. Handy for formatting data structures or removing redundant information.
...
💡 GDB Tip #56: By default when a program forks, GDB will debug the parent; the child process will run unimpeded. If you use 𝚜𝚎𝚝 𝚏𝚘𝚕𝚕𝚘𝚠-𝚏𝚘𝚛𝚔-𝚖𝚘𝚍𝚎 𝚌𝚑𝚒𝚕𝚍 (rather than 𝚙𝚊𝚛𝚎𝚗𝚝, the default) then when the program forks you will be debugging the child process.
...
💡 GDB Tip #57: Run 𝚍𝚒𝚜𝚙𝚕𝚊𝚢 [𝚎𝚡𝚙𝚛𝚎𝚜𝚜𝚒𝚘𝚗] to evaluate a particular expression each time you return to the GDB prompt. This is handy for tracking a variable over time for example, without needing to re-issue a 𝚙𝚛𝚒𝚗𝚝 command each time.
...
💡 GDB Tip #58: Sometimes an error only occurs at a certain point in the program, and so you want to stop on a breakpoint after it’s been hit a certain number of times. Using the 𝚒𝚐𝚗𝚘𝚛𝚎 command you can tell GDB to skip N breakpoint hits, allowing you to start debugging exactly when you want.
...
💡 GDB Tip #59: If you want to examine the backtrace of multiple frames but you’re very deep in the stack (so 𝚋𝚝 𝚏𝚞𝚕𝚕 would show too much), you can use 𝚋𝚊𝚌𝚔𝚝𝚛𝚊𝚌𝚎 𝚗 to show the innermost n frames.
...
💡 GDB Tip #61: Did you know you can save breakpoints to a file with 𝚜𝚊𝚟𝚎 𝚋𝚛𝚎𝚊𝚔𝚙𝚘𝚒𝚗𝚝𝚜 [𝚏𝚒𝚕𝚎𝚗𝚊𝚖𝚎], allowing you to quickly load them all again in a future debugging session using 𝚜𝚘𝚞𝚛𝚌𝚎 [𝚏𝚒𝚕𝚎𝚗𝚊𝚖𝚎].
...
💡 GDB Tip #63: GDB: Display the memory contents at a given address using 𝚡 [𝚊𝚍𝚍𝚛𝚎𝚜𝚜]. (Short for ‘examine’.) You can customise the output with an optional format and length argument too, with x/[length][format] [address].
...
💡 GDB Tip #65: TUI mode top tip 👉 If you press 𝚌𝚝𝚛𝚕 + 𝚡, then 𝚜 in TUI mode (get there with 𝚝𝚞𝚒 𝚎𝚗𝚊𝚋𝚕𝚎), it enables “SingleKey” mode. In this mode you can use 𝚛 to 𝚛𝚞𝚗, 𝚌 to 𝚌𝚘𝚗𝚝𝚒𝚗𝚞𝚎, 𝚜 for 𝚜𝚝𝚎𝚙 (and 𝚚 to leave SingleKey mode), without pressing enter.
...
💡 GDB Tip #68: Historically GDB could be known for being slow to load big programs, but recent versions of GDB allow multithreaded debuginfo and symbol parsing. This can be controlled with 𝚖𝚊𝚒𝚗𝚝 𝚜𝚎𝚝 𝚠𝚘𝚛𝚔𝚎𝚛-𝚝𝚑𝚛𝚎𝚊𝚍𝚜 [𝚗], but by default is unlimited, allowing GDB to choose what it thinks is reasonable.
...
💡 GDB Tip #70: Helgrind is a tool inside Valgrind for automatically catching common multithreading issues, like deadlocks and data races. Like Valgrind, it gives detail about what the error is and where it happened too - use it with 𝚟𝚊𝚕𝚐𝚛𝚒𝚗𝚍 --𝚝𝚘𝚘𝚕=𝚑𝚎𝚕𝚐𝚛𝚒𝚗𝚍 ./𝚖𝚢𝚙𝚛𝚘𝚐
...
💡 GDB Tip #71: If you don’t want to step into certain functions, use the skip command. e.g.
...
💡 GDB Tip #72: Finding unexpected values in your variables? 
...
💡 GDB Tip #73: Want to step over function calls?
...
💡 GDB Tip #74: Don't forget, you have OTHER open source debugging tools beyond GDB at your disposal
...
💡 GDB Tip #75: Did you know you can debug with GDB using AddressSanitizer! 
