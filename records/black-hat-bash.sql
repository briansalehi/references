...
call create_resource('Black Hat Bash', 'book', null);
...
call create_note('Black Hat Bash', 'Chapter 1', 'Get the list of environment variables?');
...
- `UID`
- `SHELL`', 'text', 'txt');
call create_note('Black Hat Bash', 'Chapter 1', 'What are the useful environment variables?');

insert into temp_blocks values ('ls', 'code', 'sh'), ('ls -l', 'code', 'sh'), ('ls -la', 'code', 'sh');
call create_note('Black Hat Bash', 'Chapter 1', 'List contents of current directory?');

insert into temp_blocks values ('mkdir /tmp/sample', 'code', 'sh');
call create_note('Black Hat Bash', 'Chapter 1', 'Create a directory?');

insert into temp_blocks values ('ps', 'code', 'sh'), ('ps -f', 'code', 'sh'), ('ps -f --forest', 'code', 'sh');
call create_note('Black Hat Bash', 'Chapter 1', 'List active processes running under current shell?');

insert into temp_blocks values ('ps -e', 'code', 'sh'), ('ps -ef', 'code', 'sh'), ('ps -ef --forst', 'code', 'sh');
call create_note('Black Hat Bash', 'Chapter 1', 'List active processes of whole system?');
...
call create_note('Black Hat Bash', 'Chapter 1', 'What is the shebang line in bsah script?');

insert into temp_blocks values ('chmod +x script.sh', 'code', 'sh');
call create_note('Black Hat Bash', 'Chapter 1', 'Make a shell script file executable?');

insert into temp_blocks values ('bash -n script.sh', 'code', 'sh');
call create_note('Black Hat Bash', 'Chapter 1', 'Dry run a script without executing commands for debugging?');

insert into temp_blocks values ('bash -x script.sh', 'code', 'sh');
call create_note('Black Hat Bash', 'Chapter 1', 'Run a script by printing line by line execution for debugging?');

insert into temp_blocks values ('', 'text', 'txt'), ('', 'code', 'sql'), ('', 'text', 'txt');
call create_note('Black Hat Bash', 'Chapter 1', '');


-- end of records
call set_section_as_complete('', ''); -- subject_name, topic_name
