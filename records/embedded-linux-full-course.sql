...
insert into temp_blocks values ('Target is a name possibly with modifiers.', 'text', 'txt');
insert into temp_blocks values ('bitbake <target>', 'code', 'sh');
insert into temp_blocks values ('bitbake ncurses', 'code', 'sh');
insert into temp_blocks values ('bitbake ncurses-native', 'code', 'sh');
...
insert into temp_blocks values ('bitbake -c <task> <recipe>', 'code', 'sh');
insert into temp_blocks values ('bitbake -c listtasks ncurses', 'code', 'sh');
insert into temp_blocks values ('bitbake -c listtasks virtual/kernel', 'code', 'sh');
insert into temp_blocks values ('bitbake -c menuconfig virtual/kernel', 'code', 'sh');
...
insert into temp_blocks values ('Lists all available recipes with their versions', 'text', 'txt');
insert into temp_blocks values ('bitbake -s', 'code', 'sh');
call flashback.create_note_with_name('Embedded Linux Full Course by Anisa Institute', 'Course 6', 'List all recipes?');

insert into temp_blocks values ('Forces the given task to be run by removing its stamp file.', 'text', 'txt');
insert into temp_blocks values ('bitbake -f', 'code', 'sh');
insert into temp_blocks values ('bitbake -f dropbear', 'code', 'sh');
...
insert into temp_blocks values ('Keyword for all recipes.', 'text', 'txt');
insert into temp_blocks values ('bitbake world', 'code', 'sh');
...
insert into temp_blocks values ('bitbake --runall=fetch core-image-minimal', 'code', 'sh');
...
insert into temp_blocks values ('Bitbake stores the output of each task in a directory within `SSTATE_CACHE` variable which defaults to `build/sstate-cache`.', 'text', 'txt');
insert into temp_blocks values ('After some time this cache direcctory grows unnecessarily, you can clear files inactive more than 30 days:', 'text', 'txt');
insert into temp_blocks values ('find build/sstate-cache -type f -atime +30 -delete', 'code', 'sh');
...
insert into temp_blocks values ('Each package recipe has a `.inc` file like `tar.inc` which contains version agnostic recipe information like `SRC_URI`.', 'text', 'txt');
insert into temp_blocks values ('For each release, packages have another file with the same name appended by an underscore version like `tar_1.26.bb`.', 'text', 'txt');
...
insert into temp_blocks values ('The first line usually is `require package.inc` which includes version agnostic file for that package.', 'text', 'txt');
insert into temp_blocks values ('Licenses are separated into version dependent recipe files.', 'text', 'txt');
...
insert into temp_blocks values ('Run and log files are generated in `temp` directory under the recipe working directory.', 'text', 'txt');
insert into temp_blocks values ('`run.do_<task_name>` and `log.do_<task_name>`.', 'text', 'txt');
...
insert into temp_blocks values ('bitbake-getvar GNU_URI', 'code', 'sh');
call flashback.create_note_with_name('Embedded Linux Full Course by Anisa Institute', 'Course 6', 'Dump a global variables?');

insert into temp_blocks values ('bitbake-getvar -r ncurses SRC_URI', 'code', 'sh');
...
insert into temp_blocks values ('bitbake -e ncurses', 'code', 'sh');
...
insert into temp_blocks values ('bitbake-getenv -r tar FILE', 'code', 'sh');
insert into temp_blocks values ('bitbake-getenv -r tar-native FILE', 'code', 'sh');
call flashback.create_note_with_name('Embedded Linux Full Course by Anisa Institute', 'Course 6', 'Locate a recipe file?');

insert into temp_blocks values ('bitbake-getenv -r tar PN', 'code', 'sh');
insert into temp_blocks values ('bitbake-getenv -r tar-native PN', 'code', 'sh');
call flashback.create_note_with_name('Embedded Linux Full Course by Anisa Institute', 'Course 6', 'Get package name of a recipe?');

insert into temp_blocks values ('The `bitbake` build engine allows to modify a recipe by extending it.', 'text', 'txt');
insert into temp_blocks values ('If we already have a recipe `meta/recipes-core/init-ifupdown/init-ifupdown_1.0.bb`,
we can extend it by writing a recipe with the same name but with `.bbappend` extension.', 'text', 'txt');
insert into temp_blocks values ('`meta-custom/recipes-core/init-ifupdown/init-ifupdown_1.0.bbappend`', 'text', 'txt');
...
insert into temp_blocks values ('Append files must have the same root name as the recipe they extend', 'text', 'txt');
insert into temp_blocks values ('Wildcards can be used in extension name but only as minor version:', 'text', 'txt');
insert into temp_blocks values ('meta-custom/recipes-core/init-ifupdown/init-ifupdown_1.%.bbappend', 'code', 'sh');
insert into temp_blocks values ('Above extension applys to all minor versions of `init-ifupdown` version 1.', 'text', 'txt');
insert into temp_blocks values ('The `%` works only just before the `.bbappend` suﬀix', 'text', 'txt');
...
insert into temp_blocks values ('`FILESEXTRAPATHS`', 'text', 'txt');
insert into temp_blocks values ('Files are looked up in paths referenced in FILESEXTRAPATHS, from left to right.', 'text', 'txt');
insert into temp_blocks values ('Prepending a path makes sure it has priority over the recipe''s one.', 'text', 'txt');
insert into temp_blocks values ('FILESEXTRAPATHS:prepend := "${THISDIR}/files:"', 'code', 'bb');
insert into temp_blocks values ('This is analogues to the following which should not be done:', 'text', 'txt');
insert into temp_blocks values ('SRC_URI += "file://custom-modification-0.patch \
...
insert into temp_blocks values ('do_install:append() {
...
insert into temp_blocks values ('Through the `PROVIDES` variable.', 'text', 'txt');
...
insert into temp_blocks values ('Classes provide and abstraction to common code in recipes.', 'text', 'txt');
insert into temp_blocks values ('Classes extension is `.bbclass` and they are located in `classes` folder of a layer.', 'text', 'txt');
...
insert into temp_blocks values ('inherit <class>', 'code', 'bb');
insert into temp_blocks values ('A recipe can inherit from multiple classes.', 'text', 'txt');
...
insert into temp_blocks values ('- `base.bbclass`
- `kernel.bbclass`
- `autotools.bbclass`
- `cmake.bbclass`
- `meson.bbclass`
...
insert into temp_blocks values ('`base.bbclass`', 'text', 'txt');
...
insert into temp_blocks values ('Tasks are chained using dependencies and they will be called in hierarchy. All of them are default implemented in base class:', 'text', 'txt');
insert into temp_blocks values ('`fetch`, `unpack`, `patch`, `configure`, `build`, `install`, and other manual tasks like `clean` and `listtasks`.', 'text', 'txt');
insert into temp_blocks values ('Base class automatically applies patch files listed in `SRC_URI`.', 'text', 'txt');
insert into temp_blocks values ('Defines mirrors like `SOURCEFORGE_MIRROR`, `DEBIAN_MIRROR`, `GNU_MIRROR`, `KERNEL_MIRROR`.', 'text', 'txt');
insert into temp_blocks values ('Defines `oe-runmake`, using `EXTRA_OEMAKE` to use custom arguments.', 'text', 'txt');
...
insert into temp_blocks values ('When we want to change make parameters without applying patches.', 'text', 'txt');
...
insert into temp_blocks values ('Defines tasks to configure, compile, install a kernel and its modules.', 'text', 'txt');
insert into temp_blocks values ('SRC_URI += file://defconfig', 'code', 'bb');
insert into temp_blocks values ('The kernel is devided into several packages like `kernel`, `kernel-base`, `kernel-dev`, `kernel-modules`, etc.', 'text', 'txt');
insert into temp_blocks values ('Automatically provides `virtual/kernel`.', 'text', 'txt');
insert into temp_blocks values ('Configuration variables like `KERNEL_IMAGETYPE`, `KERNEL_EXTRA_ARGS`, `INITRAMFS_IMAGE`, etc. are availabe.', 'text', 'txt');
call flashback.create_note_with_name('Embedded Linux Full Course by Anisa Institute', 'Course 6', 'What does kernel class do?');

insert into temp_blocks values ('`do_configure` generates configurations, `do_build` runs `make`, `do_install` runs `make install`.', 'text', 'txt');
insert into temp_blocks values ('Compilations flags can be added using `EXTRA_OEMAKE`.', 'text', 'txt');
insert into temp_blocks values ('DESCRIPTION = "Custom recipe"
...
insert into temp_blocks values ('Adds users to the resulting image.', 'text', 'txt');
insert into temp_blocks values ('`USERADD_PACKAGES` must be defined when the `useradd` class is inherited.', 'text', 'txt');
insert into temp_blocks values ('At least one of the `USERADD_PARAM` or `GROUPADD_PARAM` variables must be set.', 'text', 'txt');
insert into temp_blocks values ('Users and groups are created before the packages perform `do_install`.', 'text', 'txt');
call flashback.create_note_with_name('Embedded Linux Full Course by Anisa Institute', 'Course 6', 'What does useradd class do?');

insert into temp_blocks values ('`USERADD_PACKAGES` defines individual packages produced by the recipe that need users or groups to be added.', 'text', 'txt');
insert into temp_blocks values ('DESCRIPTION = "Custom recipe"
HOMEPAGE = "https://example.com"
SECTION = "examples"
LICENSE  = "BSD"
...
insert into temp_blocks values ('`bin_package.class` simplifies this process.', 'text', 'txt');
insert into temp_blocks values ('In this class `do_configure` and `do_compile` tasks are disabled.', 'text', 'txt');
insert into temp_blocks values ('`do_install` task copies whatever is in `S`.', 'text', 'txt');
insert into temp_blocks values ('Using this class, `LICENSE` should be set to `CLOSED` if applicable.', 'text', 'txt');
insert into temp_blocks values ('You should probably also inherit `allarch`.', 'text', 'txt');
...
insert into temp_blocks values ('`BBPATH`', 'text', 'txt');
...
insert into temp_blocks values ('`inherit`, `include`, and `require`.', 'text', 'txt');
insert into temp_blocks values ('`inherit` can be used in recipes or classes to inherit the functionality of a class.', 'text', 'txt');
insert into temp_blocks values ('`include` and `require` keywords can be used in all files to insert the content of another file at that location.', 'text', 'txt');
...
insert into temp_blocks values ('Inheriting in configuration files is based on the `INHERIT` variable.', 'text', 'txt');
insert into temp_blocks values ('INHERIT += "rm_work"', 'code', 'bb');
...
insert into temp_blocks values ('`include` does not produce an error when a file cannot be found, whereas `require` raises a parsing error.', 'text', 'txt');
insert into temp_blocks values ('require ninviders.inc
...
insert into temp_blocks values ('bitbake -c devshell <recipe>', 'code', 'sh');
...
insert into temp_blocks values ('To understand what changes after a build, enable build history in `local.conf`:', 'text', 'txt');
insert into temp_blocks values ('INHERIT += "buildhistory"
insert into temp_blocks valuesLDHISTORY_COMMIT = 1', 'code', 'conf');
insert into temp_blocks values ('buildhistory-diff', 'code', 'sh');
call flashback.create_note_with_name('Embedded Linux Full Course by Anisa Institute', 'Course 6', 'Differenciate two builds?');

insert into temp_blocks values ('`DL_DIR`, `PREMIRRORS`, `SRC_URI`, `MIRRORS`.', 'text', 'txt');
insert into temp_blocks values ('If all mirrors fail, build will fail.', 'text', 'txt');
...
insert into temp_blocks values ('`own-mirrors` can only add one URL:', 'text', 'txt');
insert into temp_blocks values ('INHERITS += "own-mirrors"
SOURCE_MIRROR_URL = "http://example.com/my-mirror"', 'code', 'bb');
insert into temp_blocks values ('For a more complex setup, prepend custom mirrors to the `PREMIRRORS` variable:', 'text', 'txt');
insert into temp_blocks values ('PREMIRRORS:prepend = "\
...
insert into temp_blocks values ('BB_GENERATE_TARBALLS = "1"', 'code', 'bb');
call flashback.create_note_with_name('Embedded Linux Full Course by Anisa Institute', 'Course 6', 'Create a local mirror?');

insert into temp_blocks values ('`do_fetch` is only network enabled to make sure no untraced sources are fetched.', 'text', 'txt');
...
insert into temp_blocks values ('BB_NO_NETWORK = "1"', 'code', 'bb');
call flashback.create_note_with_name('Embedded Linux Full Course by Anisa Institute', 'Course 6', 'Complete network access?');

insert into temp_blocks values ('bitbake --runall=fetch core-image-minimal', 'code', 'sh');
insert into temp_blocks values ('Or restrict bitbake to only download files from the `PREMIRRORS`:', 'text', 'txt');
insert into temp_blocks values ('BB_FETCH_PREMIRRORONLY = "1"', 'code', 'bb');
...
insert into temp_blocks values ('It is a good practice to begin a layer name with the prefix `meta-`.', 'text', 'txt');
...
insert into temp_blocks values ('Recipes and metadata.', 'text', 'txt');
...
insert into temp_blocks values ('https://layers.openembedded.org', 'text', 'link');
...
insert into temp_blocks values ('The list of layers `bitbake` uses is defined in `$BUILDDIR/conf/bblayers.conf`.', 'text', 'txt');
...
insert into temp_blocks values ('To include a new layer, add its absolute path to the `BBLAYERS` variable.', 'text', 'txt');
insert into temp_blocks values ('`bitbake` parses each layer from `BBLAYERS` and adds the recipes, configuration files and classes it contains.', 'text', 'txt');
call flashback.create_note_with_name('Embedded Linux Full Course by Anisa Institute', 'Course 6', 'Include another layer?');

insert into temp_blocks values ('bitbake-layers', 'code', 'sh');
...
insert into temp_blocks values ('bitbake-layers show-layers', 'code', 'sh');
call flashback.create_note_with_name('Embedded Linux Full Course by Anisa Institute', 'Course 6', 'Get a list of layers?');

insert into temp_blocks values ('bitbake-layers add-layer meta-custom', 'code', 'sh');
call flashback.create_note_with_name('Embedded Linux Full Course by Anisa Institute', 'Course 6', 'Add a custom layer?');

insert into temp_blocks values ('bitbake-layers remove-layer meta-qt5', 'code', 'sh');
call flashback.create_note_with_name('Embedded Linux Full Course by Anisa Institute', 'Course 6', 'Remove a custom layer?');

insert into temp_blocks values ('Many SoC layers exist providing support for boards.', 'text', 'txt');
insert into temp_blocks values (
'- `meta-ti-bsp`
- `meta-freescale`
- `meta-st-stm32mp`', 'text', 'txt');
insert into temp_blocks values ('Many layers offer to support applications not available in poky reference system, e.g:', 'text', 'txt');
insert into temp_blocks values (
...
insert into temp_blocks values ('A layer is a set of files and directories and can be created by hand but best practice is to use `bitbake-layers`:', 'text', 'txt');
insert into temp_blocks values ('bitbake-layers create-layer -p <priority> <layer>', 'code', 'sh');
insert into temp_blocks values ('The **priority** is used to select which recipe to use when multiple layers contains the same recipe.', 'text', 'txt');
insert into temp_blocks values ('Layer priority takes precedence over the recipe version number ordering. This allows to downgrade a recipe in a layer.', 'text', 'txt');
